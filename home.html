...
<head>
  ...
  <style>
    ...
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background:#f2f2f2; }header { background:#fff; box-shadow:0 2px 6px rgba(0,0,0,.08); padding:10px 16px; }
...
.toast { position:fixed; bottom:24px; left:50%; transform:translate(-50%,20px); background:#f44336; color:#fff; padding:12px 20px; border-radius:8px; font-size:14px; opacity:0; pointer-events:none; z-index:1000; transition:opacity .4s ease, transform .4s ease; box-shadow:0 4px 12px rgba(0,0,0,.2); }
...
.loading-screen {
  position:fixed; top:0; left:0; width:100%; height:100%; background:#f2f2f2;
  display:none; align-items:center; justify-content:center; font-size:18px;
  color:#3e723a; z-index:9999;
}
...
<div id="loading" class="loading-screen">جاري التحميل...</div><!-- باقي الجسم كما هو --><!-- ... اختصرنا هنا لأجل التركيز على التعديلات المطلوبة ... --><script>
  // حفظ الصفحة الحالية كمصدر
  ...
  // الرجوع للمكان الصحيح عند العودة من صفحة أخرى
  ...
  document.querySelectorAll('.bank-card').forEach(card => {
    ...
    card.addEventListener('touchend', e => {
      ...
      const maxMove = 10;
      if (deltaX < maxMove && deltaY < maxMove) animateAndRedirect();
    });
  });
</script></body>
</html>
